...
int build_prefix_table(const std::string &pattern, std::vector<size_t> &table)
{
    ...
    for (off += 1, *p_prefix = 0, p_prefix += 1; off < last; off += 1, p_prefix += 1) {
        *p_prefix = 0;
        ...
        for (; off_last1 > 0; off_last1 -= 1) {
            ...
            std::string s2(off + 1 - off_last1,
                off + 1); // use [first,last) to constructor // not overflow
            if (s1 == s2) {
                *p_prefix = off_last1;
                break;
            }
...
int build_prefix_table2(const std::string &pattern, std::vector<size_t> &table)
{
    table.clear();
    table.resize(pattern.size());
...
int build_prefix_table3(const std::string &pattern, std::vector<size_t> &table)
{
    table.clear();
    table.resize(pattern.size());
...
int build_prefix_table4(const std::string &pattern, std::vector<size_t> &table)
{
    table.clear();
    table.resize(pattern.size());
...
size_t kmp_search(const std::string &pattern, const std::string &s)
{
    ...
    const char *last1 = first1 + s.size(); // first1 + s.size() +1 - pattern.size();
