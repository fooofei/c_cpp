...
// 注意，建立最大堆/最小堆的过程需要循环下沉， 最大堆/最小堆
// 是父节点都比叶子大/小的堆。下面的函数建立的堆，不符合这个要求。
void heapify(int *begin, int *end)
{
    int n;
    for (n = (int)(end - begin - 1); n > 1; --n) {
        if (*(begin + n) < *(begin + n / 2)) {
            swap((begin + n), (begin + n / 2));
        }
    }
}
...
int main()
{
    ...
    int ar_top[k+1]={0}; // 0 索引不用，方便父子关系的计算

    // 先填充 k 个值
    size_t j = 0;
    size_t i;
    for (i = 1; i < _countof(ar_top); ++i, ++j) {
        ar_top[i] = ar[j];
    }
