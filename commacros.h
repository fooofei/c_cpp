...
inline void lock_increment(volatile unsigned long *ref)
{
#ifdef WIN32
...
inline void lock_decrement(volatile unsigned long *ref)
{
#ifdef WIN32
...
#define ADDREF STDMETHOD_(unsigned long, AddRef)() \
    {                                   \
        lock_increment(&m_ref);         \
        return m_ref;                   \
    }
#define RELEASE STDMETHOD_(unsigned long, Release)() \
    {                                    \
        assert(0 < m_ref);               \
        lock_decrement(&m_ref);          \
        unsigned long ref = m_ref;       \
        if (0 == ref) {                  \
            delete this;                 \
        }                                \
        return ref;                      \
    }
#define IBEGIN STDMETHOD(QueryInterface)(REFIID iid, void **ppv) \
    {                                                 \
        *ppv = NULL;
#define IUNKNOWN if (IID_IUnknown == iid) {                                           \
        *ppv = static_cast<IUnknown *>(static_cast<UnknownImp *>(this)); \
        this->AddRef();                                                  \
        return S_OK;                                                     \
    }
#define I(Ix) if (IID_##Ix == iid) {              \
