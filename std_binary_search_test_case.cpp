...
void _test_std_framework(const uint32_t *b_dst, const uint32_t *e_dst, const uint32_t *b_search,
    const uint32_t *e_search, const uint32_t *(*pfn)(const uint32_t *, const uint32_t *, const uint32_t &))
{
    const uint32_t *p;
    const uint32_t *r;
...
void _test1()
{
    ...
    _test_std_framework(ARRAY_BEGIN_END(array) - 1, ARRAY_BEGIN_END(array_search), std::lower_bound);
}
...
void _test2()
{
    ...
    uint32_t array_search[] = {
        1, (uint32_t)(ARRAY_END(array) - 1 - array - 1) // NOT
        , 
        6, 0 // NOT
        , 
        3, 0
...
void _test3()
{
    ...
    _test_std_framework(ARRAY_BEGIN_END(array) - 1, ARRAY_BEGIN_END(array_search), std::upper_bound);
}
