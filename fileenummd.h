...
namespace path{
	...
	//  3. 无任务，但是有 active thread (active_thread_count > 0), 那么就在  WaitForSingleObject(event_not_empty,INFINITE)
	//  4. 无任务，而且  active_thread_count ==0 ， 那么就退出工作，并且 SetEvent(event_not_empty); WaitForSingleObject(event_not_empty,0) != WAIT_TIMEOUT
	//  			这段代码，先置受信，然后等待，如果没有拿走这个置信的状态，也就是没有其他线程在 event_not_empty 上 wait， 说明这是最后一个线程，后面的代码会立刻返回，
