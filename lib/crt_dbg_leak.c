...
// 这里有一份可以参考的实现
// https://github.com/piscisaureus/wepoll/blob/master/test/shared/leak-check.c
...
void crt_dbg_leak_unlock(struct _crt_dbg_leak *self)
{
    ...
    if (self->oldflag) {
        ...
        xxx.dll!__DllMainCRTStartup(void * hDllHandle, unsigned long dwReason, void * lpreserved) 行 390	C
        已加载符号。 >	xxx.dll!_DllMainCRTStartup(void * hDllHandle, unsigned long dwReason, void * lpreserved) 行 331
        C	已加载符号。
        ...
        > FreeLibrary()
